...
void PasswordResetDialog::onCurrentPasswordChanged() {
    updateButtonState();
}

void PasswordResetDialog::onNewPasswordChanged() {
    ...
    if (!password.isEmpty()) {
        ...
        if (password.length() < 6) {
            ...
            passwordStatus->setText("密码长度不能超过50位");
        } else {
...
void PasswordResetDialog::onConfirmPasswordChanged() {
    ...
    if (!confirmPassword.isEmpty() && !newPassword.isEmpty() && newPassword != confirmPassword) {
        ...
        passwordStatus->setText("请先输入新密码");
    } else {
        // Re-validate password if confirm matches
...
void PasswordResetDialog::updateButtonState() {
    QString currentPassword = currentPasswordEdit->text();
    QString newPassword = newPasswordEdit->text();
    QString confirmPassword = confirmPasswordEdit->text();
    
    // Enable reset button only if all fields are valid
    bool isValid = !currentPassword.isEmpty() &&
                   !newPassword.isEmpty() && newPassword.length() >= 6 && newPassword.length() <= 50 &&
