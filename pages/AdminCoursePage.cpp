...
AdminCoursePage::AdminCoursePage(QWidget *parent) : QWidget(parent) {
    ...
    titleLabel = new QLabel("课程管理", this);
        ...
        if (Course *course = aaims::manager::course::get_courses()[tableModel->getCourse(
            ...
            if (result == QMessageBox::Yes) {
                ...
                connect(watcher, &QFutureWatcherBase::finished, [this, pd, watcher] {
                    ...
                    pd->deleteLater();
                    watcher->deleteLater();
                    QMessageBox::information(this, "删除完成", QString("删除课程成功！"));
                });
...
void AdminCoursePage::reloadData() const {
    tableModel->setCourses(aaims::manager::course::get_courses().keys());
    proxyModel->sort(0);
    subtitleLabel->setText(QString("管理系统内共 %1 个课程").arg(tableModel->rowCount(QModelIndex())));
}
